...
export function MenuItemCard({ item, onAddToCart, globalShowImages = true }: MenuItemCardProps) {
  ...
  return (
    <div className={cn(
      "group relative bg-zinc-900/60 rounded-[2rem] overflow-hidden transition-all duration-500",
      "border border-primary/20 shadow-2xl backdrop-blur-sm",
      isSoldOut ? "opacity-60" : "hover:border-primary/50 hover:-translate-y-1 hover:shadow-primary/10",
        ...
        <div className="relative h-64 w-full overflow-hidden">
          {isSoldOut && (
            <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/80 backdrop-blur-[2px]">
              <div className="bg-zinc-800 text-zinc-400 px-6 py-2 rounded-full border border-zinc-700 font-bold uppercase tracking-widest text-[10px]">
                Sold Out
              </div>
            </div>
          )}
          ...
          {!isSoldOut && (
            <div className="absolute top-4 right-4 z-10">
              <div className="bg-black/60 backdrop-blur-md px-4 py-1.5 rounded-full border border-primary/30 flex items-center gap-1.5">
                <Star size={10} className="text-primary fill-primary" />
                <span className="text-primary font-bold text-xs">Featured</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="p-6 space-y-4">
        <div className="space-y-2">
          <div className="flex justify-between items-start gap-4">
            ...
            <span className="text-primary font-black text-lg tabular-nums shrink-0">
        ...
        <button
          ...
          className={cn(
            ...
            isSoldOut 
              ...
              : "bg-primary text-black shadow-lg shadow-primary/20 hover:bg-white hover:shadow-white/10 active:scale-95"
          )}
        >
          {isSoldOut ? (
            'OUT OF STOCK'
